...
pub fn applyProfile(catalog: *const model.Catalog, target: *model.ProfileConfig, key: []const u8, text: ?[]const u8) !void {
    ...
    if (eq(key, "diskless.failure.backoff_seconds")) {
        target.diskless.failure.backoff_seconds = try parseInt(u32, text);
        return;
    }
...
fn parseBool(value: ?[]const u8) !bool {
    ...
    if (eq(text, "yes")) return true;
    if (eq(text, "false")) return false;
    if (eq(text, "no")) return false;
    return error.InvalidPropertyValue;
}
fn parseInt(comptime T: type, value: ?[]const u8) !T {
    return std.fmt.parseInt(T, value orelse return error.PropertyRequired, 10) catch error.InvalidPropertyValue;
}
