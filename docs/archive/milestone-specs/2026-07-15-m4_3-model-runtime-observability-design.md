...
- `deployment_armed_at`：generation arm/request 时间，是整个部署任务的 Armed；
...
management API JSON 返回 `armed_at`/`install_at`/`finished_at`，并在详情中保留最近成功的 `deployed_at`。
...
| `src/cli/views.zig` | 新 node/profile list 列和完整 show 分组；`formatTimestamp` 可视化时间；node list 增 ARMED/INSTALL/FINISHED 列 |
