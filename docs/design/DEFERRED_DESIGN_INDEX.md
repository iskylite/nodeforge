...
| `ENV-V041-KERNEL-VERMAGIC` / `V041-D09` | 换核后 diskless **节点侧** vermagic / Lustre-class 实机闭环：树内新 `vmlinuz`+`/lib/modules/<release>` → `--from-staging --kernel-release` 更新启动面 → 节点 PXE 启动后模块与运行中内核 vermagic 对齐（含 Lustre 或等价驱动场景） | 当前无法在可信目标环境完成「管理节点换核导入 + 计算节点 diskless 启动 + 模块加载」端到端矩阵；不阻断 v0.4.1 产品路径与管理面契约闸 | v0.4.1 已交付：`staging enter/exec`、`kernels` 扫描、`--kernel-release keep\|auto\|<release>` 启动面导入、from-staging 先 import 再 publish、契约/单元测试与管理面 catalog/boot_bundle 更新证据；推荐操作序见 [`V0_4_1_DESIGN.md`](V0_4_1_DESIGN.md) §6.5 | 在 Linux 管理节点 + 可 PXE diskless 计算节点（或等价实验室矩阵）上按设计 §6.5 操作序完成至少一次：树内新 release + 模块 → 启动面导入 → 节点启动后 `modinfo`/加载驱动 vermagic 对齐；写入独立验证记录（候选版本、环境、命令、日志、结果）后解除 |
...
| rootfs staging 会话（enter/exec、cgroup 挂载与限额、树内核导入） | **已实现（v0.4.1）** | [`V0_4_1_DESIGN.md`](V0_4_1_DESIGN.md) | 正式 CLI 为 `profile rootfs staging …`；**含** cgroup 限额参数；**含** `--kernel-release` 选择与启动面导入；**不含** `pivot_root` 与顶层 `rootfs` 命令。管理面/契约验证见 [`V0_4_1_VALIDATION_RUNBOOK.md`](../validation/V0_4_1_VALIDATION_RUNBOOK.md)；**节点侧 vermagic/Lustre 实机闭环**见 `ENV-V041-KERNEL-VERMAGIC` | v0.4.1 完成标准见该设计 §10（不含已延期的节点侧 vermagic 证据） |
