...
| BootConfig / AgentPlan / initrd / agent | v0.4 BootConfig v3 + AgentPlan v2、固定 squashfs mount | BootConfig v4 增 mode/expanded size；AgentPlan v2 与 agent 拉取/执行协议不变 |
...
**两种形态沿用同一 BootConfig 投递方法，但 v0.5 DTO 升级为 v4**。BootConfig 是 per-boot、per-Node 的短时 DTO，由服务端按
...
（`ram_rootfs` 没有 overlay upper，直接写内存根）。BootConfig v4 在 v0.4 的 v3 基础上给 `overlay` 增加必填 `mode`，给 `rootfs` 增加必填
`uncompressed_size`；token 边界不变。全部 v0.5 delivery 使用 v4，因此 BootConfig v3 及更早 initrd 在 readiness
...
ram-only feature，但其 initrd 仍必须声明支持 BootConfig v4 schema。
...
v0.4 升级前已创建的 active/recoverable BootConfig v3 session 继续消费自己的 immutable v3 snapshot 直到终态；上述
“v3 及更早 readiness 拒绝”只适用于新 v0.5 delivery，升级不能重编译、重签或终止旧 session。

## 4. 通用与差异

**通用（rootfs 物化之外全部一致）**：

- BootConfig 生成/投递方法、capability token、secret 边界（DTO schema 由 v3 升 v4）。
...
- schema 版本：v0.5 rootfs 形态字段对应 catalog schema v7（v0.4 使用 v6）；BootConfig DTO 从 v3 升 v4，
...
- `boot preview` 的 capsule/kernel/initrd 路径不变，但 JSON 必须显示 `bootconfig_schema=4`、`overlay_mode`、
...
- v0.4 -> v0.5 迁移：schema v6 到 v7 的 plan/apply 将现有 diskless Profile 默认补
  ...
  `migration.non_representable` 并列出 resource/path/reason/target_schema。active/recoverable BootConfig v3/v4 snapshot
...
- v0.5 两种形态下 token/effective plan/agent node-apply/first-boot 行为一致；BootConfig v4 显式携带 mode/expanded size，
...
- 新 v0.5 delivery 面对 BootConfig v3 及更早 initrd、缺可信 uncompressed size 时在 boot readiness 阶段 fail closed；
  升级前已存在的 active/recoverable v3 session 必须继续到终态。`mode=ram_rootfs` 时
...
- v6 -> v7 正向迁移、finalize 前 rollback、全 `squashfs_overlay` 可表示 downgrade、含 `ram_rootfs` 不可表示 downgrade
  和 active v3 session 跨升级均有 fixture。
