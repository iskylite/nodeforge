...
fn parseArgs(self: *Command, argsIterator: *std.process.Args.Iterator) CommandParseErrors!ParserOutput { // needs to give back a cmd context
    ...
    outer: while (argsIterator.next()) |arg| {
        ...
        switch (arg_type) {
            .WORD => {
                if (current_cmd.findCommand(arg)) |found_cmd| {
                    found_cmd.checkDeprecated() catch |err| {
                        try self.init_options.writer.flush();
                        return err;
                    };

                    // NodeForge vendor patch: v5.1.2 copies persistent flag
                    // definitions to descendants, but not values parsed before
                    // the subcommand. Carry those values into the selected child.
                    var flag_it = current_cmd.flags_by_name.valueIterator();
                    while (flag_it.next()) |flag| {
                        if (!flag.persistent) continue;
                        if (current_cmd.flag_values.get(flag.name)) |value| {
                            try found_cmd.flag_values.put(flag.name, value);
                        }
                    }
    ...
    // NodeForge vendor patch: a conventional global version flag should not
    // fail positional validation on a leaf command such as `import`.
    if (current_cmd.flag_values.get("version")) |flag_value| {
        if (flag_value == .Bool and flag_value.Bool) {
            return .{
                .program_name = prog_name,
                .command = current_cmd,
                .remaining_args = remaining_args,
